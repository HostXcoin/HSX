...
public:
    ...
    bool WriteZHSXSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZHSXSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZHSXSeed_deprecated(uint256& seed);
    bool EraseZHSXSeed();
    bool EraseZHSXSeed_deprecated();

    bool WriteZHSXCount(const uint32_t& nCount);
    bool ReadZHSXCount(uint32_t& nCount);
