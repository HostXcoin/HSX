...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZHSXStake(spend));
    } else {
        ...
        CHSXStake* HSXInput = new CHSXStake();
        HSXInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(HSXInput);
    }
