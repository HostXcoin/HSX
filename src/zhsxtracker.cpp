...
CzhsxTracker::CzhsxTracker(std::string strWalletFile)
{
...
CzhsxTracker::~CzhsxTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzhsxTracker::Init()
{
...
bool CzhsxTracker::Archive(CMintMeta& meta)
{
...
bool CzhsxTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzhsxTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzhsxTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzhsxTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzhsxTracker::GetSerialHashes()
{
...
CAmount CzhsxTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzhsxTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzhsxTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzhsxTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzhsxTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzhsxTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzhsxTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzhsxTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzhsxTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzhsxTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzhsxTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzhsxTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzhsxTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzhsxTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzhsxTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzhsxTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzhsxTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzhsx from DB\n", __func__, listDeterministicDB.size());
    }
