...
enum AvailableCoinsType {
    ...
    EXCEPT_DENOMINATED_OR_MASTERNODE_COLLATERAL = 4, // ONLY_NONDENOMINATED and not 15000 HSX at the same time
    ONLY_MASTERNODE_COLLATERAL = 5,       // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zhsx send
enum ZerocoinSpendStatus {
    ZHSX_SPEND_OKAY = 0,                            // No error
    ZHSX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZHSX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZHSX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZHSX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZHSX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZHSX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZHSX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZHSX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZHSX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZHSX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZHSX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZHSX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZHSX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZHSX_SPENT_USED_ZHSX = 14,                      // Coin has already been spend
    ZHSX_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZHSX_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZHSXBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZHSXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzhsxAuto) const;
    ...
    */
    ...
    CzhsxWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzhsxTracker> zhsxTracker;
    ...
    void setZWallet(CzhsxWallet* zwallet)
    {
        zwalletMain = zwallet;
        zhsxTracker = std::unique_ptr<CzhsxTracker>(new CzhsxTracker(strWalletFile));
    }

    CzhsxWallet* getZWallet() { return zwalletMain; }
    ...
    /** zhsx reset */
    boost::signals2::signal<void()> NotifyzhsxReset;
