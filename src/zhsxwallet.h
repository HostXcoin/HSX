...
#ifndef HostxCoin_ZHSXWALLET_H
#define HostxCoin_ZHSXWALLET_H
...
public:
    CzhsxWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZHSX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZHSX(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //HostxCoin_ZHSXWALLET_H
