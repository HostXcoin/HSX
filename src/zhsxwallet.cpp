...
CzhsxWallet::CzhsxWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zhsx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZHSXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZHSXSeed_deprecated()) {
                    LogPrintf("%s: Updated zhsx seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zhsx seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zhsx wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzhsxWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZHSXCount(nCountLastUsed);
    else if (!walletdb.ReadZHSXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzhsxWallet::Lock()
{
    seedMaster = 0;
}

void CzhsxWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzhsxWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZHSX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzhsxWallet::LoadMintPoolFromDB()
{
...
void CzhsxWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzhsxWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzhsxWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zhsxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzhsxWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZHSX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zhsxTracker which also adds to database
    pwalletMain->zhsxTracker->Add(dMint, true);
...
void CzhsxWallet::SeedToZHSX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzhsxWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzhsxWallet::GenerateDeterministicZHSX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzhsxWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzhsxWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
