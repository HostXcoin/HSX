...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::HSX);
    ...
    if (!settings.contains("nAnonymizeHostxCoinAmount"))
        settings.setValue("nAnonymizeHostxCoinAmount", 1000);

    nAnonymizeHostxCoinAmount = settings.value("nAnonymizeHostxCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeHostxCoinAmount"))
        SoftSetArg("-anonymizehostxcoinamount", settings.value("nAnonymizeHostxCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in hostxcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHostxCoinAmount:
            return QVariant(nAnonymizeHostxCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHostxCoinAmount:
            nAnonymizeHostxCoinAmount = value.toInt();
            settings.setValue("nAnonymizeHostxCoinAmount", nAnonymizeHostxCoinAmount);
            emit anonymizeHostxCoinAmountChanged(nAnonymizeHostxCoinAmount);
            break;
        case CoinControlFeatures:
