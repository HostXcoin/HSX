...
#include "ui_zhsxcontroldialog.h"
...
std::set<std::string> ZHSXControlDialog::setSelectedMints;
std::set<CMintMeta> ZHSXControlDialog::setMints;

ZHSXControlDialog::ZHSXControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZHSXControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZHSXControlDialog::~ZHSXControlDialog()
{
    delete ui;
}

void ZHSXControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZHSXControlDialog::updateList()
{
...
void ZHSXControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZHSXControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZHSX_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZHSXControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZHSXControlDialog::GetSelectedMints()
{
...
void ZHSXControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
