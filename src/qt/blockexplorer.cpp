...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::HSX, nValue);
...
static std::string makeHTMLTable(const std::string* pCells, int nRows, int nColuhsx)
{
    std::string Table = table;
    for (int i = 0; i < nRows; i++)
        Table += makeHTMLTableRow(pCells + i * nColuhsx, nColuhsx);
    Table += "</table>";
    return Table;
}
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (hostxcoin.conf).");
            QMessageBox::warning(this, "HostxCoin Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
